# 📝 Template Literal Types: Strings Dinámicos y Type-Safe

Después de dominar **Mapped Types**, ahora toca **Template Literal Types**: el superpoder para crear tipos de string dinámicos, altamente específicos y completamente type-safe.

Imagina que necesitas crear rutas API, nombres de eventos, o headers HTTP que sean validados en tiempo de compilación. Template literal types hacen exactamente eso. **Son template strings, pero para tipos.** ✨

---

## 📚 Índice de Contenidos

1. **[¿Qué Son Template Literal Types?](#qué-son-template-literal-types)** - Conceptos fundamentales
2. **[Sintaxis: `string ${Type} string`](#sintaxis-string-type-string)** - Creando strings dinámicos
3. **[Unions Generan Combinaciones](#unions-generan-combinaciones)** - Producto cartesiano
4. **[Intrinsic String Types](#intrinsic-string-types)** - Uppercase, Lowercase, Capitalize
5. **[Event Names Type-Safe](#event-names-type-safe)** - Patrones 'entity.action'
6. **[Route Definitions](#route-definitions)** - API versioning y CRUD
7. **[Query Builders Type-Safe](#query-builders-type-safe)** - Field operators
8. **[HTTP Headers & CSS Values](#http-headers--css-values)** - Validación de standards
9. **[Environment Variables](#environment-variables)** - Config type-safe
10. **[Pattern Matching & Extraction](#pattern-matching--extraction)** - `infer` con template literals
11. **[Resumen](#resumen-lo-que-aprendiste)** - Recap
12. **[Checklist](#-checklist-¿entendiste-template-literal-types)** - Validación

---

## ¿Qué Son Template Literal Types?

Un **template literal type** es un tipo que usa sintaxis de template strings para crear tipos de string altamente específicos.

Sintaxis base:

$$\`\text{string} \$\{Type\} \text{string}\`$$

**Lo clave:** Construyes tipos como construyes strings, pero validados completamente. ¡No más "cualquier string"!

**Ejemplo mental:**
```typescript
// Sin template literals: "cualquier string" ❌
type Route = string;
// '/users', '/invalid', 'xyz'... TODO VALE

// Con template literals: strings específicos ✅
type Route = '/users' | '/products' | '/orders';
// SOLO esos, nada más
```

---

## Sintaxis: `string ${Type} string`

**¿Cómo funciona?**

```typescript
// Tipo simple
type World = 'world';
type Greeting = `hello ${World}`;
// Type: 'hello world' ✅

// Con unions: genera TODAS las combinaciones
type Color = 'red' | 'blue' | 'green';
type Button = `${Color}-button`;
// Type: 'red-button' | 'blue-button' | 'green-button' ✅
```

### ¿Por Qué Importa?

Sin template literals:
```typescript
// ❌ Manual y frágil
type ButtonVariant = 'red-button' | 'blue-button' | 'green-button';
// Si agrego color, debo actualizar esto
```

Con template literals:
```typescript
// ✅ Automático y escalable
type ButtonVariant = `${Color}-button`;
// Agrego color a la union, ¡todo se genera automáticamente!
```

---

## Unions Generan Combinaciones

Cuando usas unions en template literals, TypeScript genera el **producto cartesiano** (todas las combinaciones).

```typescript
type Size = 'small' | 'medium' | 'large';
type Color = 'red' | 'blue' | 'green';

// Múltiples placeholders = combinaciones
type ButtonVariant = `${Size}-${Color}-button`;
// Type: 'small-red-button' | 'small-blue-button' | ... (9 combinaciones)

// Con 3 unions: 3 × 3 × 1 = 9 tipos diferentes ✅
```

### Caso Real: Nombres de Métodos

```typescript
type Method = 'GET' | 'POST' | 'PUT' | 'DELETE';
type Resource = 'users' | 'products' | 'orders';

type ApiMethod = `${Lowercase<Method>}${Capitalize<Resource>}`;
// Type:
//   | 'getUsers' | 'getProducts' | 'getOrders'
//   | 'postUsers' | 'postProducts' | 'postOrders'
//   | 'putUsers' | 'putProducts' | 'putOrders'
//   | 'deleteUsers' | 'deleteProducts' | 'deleteOrders' ✅
```

---

## Intrinsic String Types

TypeScript proporciona tipos built-in para manipular strings:

- `Uppercase<T>`: Convertir a MAYÚSCULAS
- `Lowercase<T>`: Convertir a minúsculas
- `Capitalize<T>`: Primera letra mayúscula
- `Uncapitalize<T>`: Primera letra minúscula

```typescript
type Upper = Uppercase<'hello'>;
// Type: 'HELLO' ✅

type Lower = Lowercase<'HELLO'>;
// Type: 'hello' ✅

type Cap = Capitalize<'hello'>;
// Type: 'Hello' ✅

type Uncap = Uncapitalize<'Hello'>;
// Type: 'hello' ✅
```

### Caso Real: HTTP Handlers

```typescript
type HTTPMethod = 'get' | 'post' | 'put' | 'delete';

// Generar nombres de handlers automáticamente
type HandlerName<M extends HTTPMethod> = `handle${Capitalize<M>}Request`;

type GetHandler = HandlerName<'get'>;
// Type: 'handleGetRequest' ✅

type PostHandler = HandlerName<'post'>;
// Type: 'handlePostRequest' ✅
```

---

## Event Names Type-Safe

Patrón común: `entity.action` para eventos de dominio.

```typescript
type DomainEntity = 'user' | 'order' | 'product';
type DomainAction = 'created' | 'updated' | 'deleted';

type EventName<E extends DomainEntity, A extends DomainAction> = `${E}.${A}`;

type UserCreated = EventName<'user', 'created'>;
// Type: 'user.created' ✅

// Generar TODOS los eventos posibles
type AllEvents = EventName<DomainEntity, DomainAction>;
// Type:
//   | 'user.created' | 'user.updated' | 'user.deleted'
//   | 'order.created' | 'order.updated' | 'order.deleted'
//   | 'product.created' | 'product.updated' | 'product.deleted' ✅
```

### Caso Real: Event Emitter Type-Safe

```typescript
interface EventPayloads {
  'user.created': { userId: string; email: string };
  'user.updated': { userId: string; changes: Record<string, any> };
  'order.created': { orderId: string; total: number };
}

class TypeSafeEventEmitter {
  emit<E extends keyof EventPayloads>(
    event: E,
    payload: EventPayloads[E],
  ): void {
    console.log(`Event: ${event}`, payload);
  }

  on<E extends keyof EventPayloads>(
    event: E,
    handler: (payload: EventPayloads[E]) => void,
  ): void {
    // Register handler
  }
}

const emitter = new TypeSafeEventEmitter();

// Type-safe emit: TypeScript valida el payload
emitter.emit('user.created', {
  userId: '123',
  email: 'john@example.com',
}); // ✅

// emitter.emit('user.created', { userId: '123' }); 
// ❌ Error: falta email (type-safe!)

// Type-safe listener: tipos del payload inferidos
emitter.on('order.created', (payload) => {
  console.log(payload.orderId); // Type: string ✅
  console.log(payload.total); // Type: number ✅
});
```

---

## Route Definitions

Rutas API type-safe con versionado y validación.

```typescript
// Rutas CRUD básicas
type Entity = 'users' | 'products' | 'orders';
type Action = 'list' | 'detail' | 'create' | 'update' | 'delete';

type RestRoute<E extends Entity, A extends Action> = A extends 'list'
  ? `/${E}`
  : A extends 'create'
  ? `/${E}`
  : `/${E}/:id`;

type UserList = RestRoute<'users', 'list'>;
// Type: '/users' ✅

type UserDetail = RestRoute<'users', 'detail'>;
// Type: '/users/:id' ✅
```

### API Versioning Type-Safe

```typescript
type ApiVersion = 1 | 2 | 3;
type VersionedRoute<V extends ApiVersion, Path extends string> = `/api/v${V}${Path}`;

type UserRouteV1 = VersionedRoute<1, '/users'>;
// Type: '/api/v1/users' ✅

type UserRouteV2 = VersionedRoute<2, '/users'>;
// Type: '/api/v2/users' ✅

// En NestJS Controller
@Controller()
export class VersionedController {
  @Get('/api/v1/users' as VersionedRoute<1, '/users'>)
  getUsersV1() {
    // V1 implementation
  }

  @Get('/api/v2/users' as VersionedRoute<2, '/users'>)
  getUsersV2() {
    // V2 implementation - cambios de schema, etc
  }
}
```

---

## Query Builders Type-Safe

Operadores de query con validación por tipo de campo.

```typescript
type Operator = 'eq' | 'ne' | 'gt' | 'lt' | 'gte' | 'lte' | 'like';

type FieldOperator<F extends string, O extends Operator> = `${F}_${O}`;

type UserAgeGt = FieldOperator<'age', 'gt'>;
// Type: 'age_gt' ✅

type UserNameLike = FieldOperator<'name', 'like'>;
// Type: 'name_like' ✅

// Query params type-safe
interface User {
  id: string;
  name: string;
  email: string;
  age: number;
  isActive: boolean;
}

type QueryOperators<T> = {
  [K in keyof T as K extends string ? `${K}_${Operator}` : never]?: T[K] extends string
    ? string
    : T[K] extends number
    ? number
    : T[K] extends boolean
    ? boolean
    : never;
};

type UserQuery = QueryOperators<User>;
// Type: {
//   id_eq?: string;
//   name_like?: string;
//   age_gt?: number;
//   age_lt?: number;
//   isActive_eq?: boolean;
//   ... (todas las combinaciones type-safe)
// } ✅

// Uso: type-safe por tipo de field
const query: Partial<UserQuery> = {
  age_gt: 18, // ✅ number
  name_like: '%John%', // ✅ string
  // age_gt: 'invalid', // ❌ Error: debe ser number
};
```

---

## HTTP Headers & CSS Values

Validación de standards web a nivel de tipos.

### HTTP Headers

```typescript
type StandardHeader =
  | 'content-type'
  | 'authorization'
  | 'accept'
  | 'user-agent';

// Headers personalizados con prefijo
type CustomHeader<Prefix extends string> = `x-${Lowercase<Prefix>}-${string}`;

type AppHeader = CustomHeader<'app'>;
// Type: `x-app-${string}` ✅

// Cualquier header válido
type AllHeaders = StandardHeader | CustomHeader<'app' | 'api'>;

const headers: Record<string, string> = {
  'content-type': 'application/json',
  'x-app-version': '1.0.0',
};

// Type-safe header access
const contentType = headers['content-type']; // ✅ conocido
const customHeader = headers['x-app-version']; // ✅ válido por patrón
```

### CSS Values

```typescript
type CSSUnit = 'px' | 'em' | 'rem' | '%' | 'vh' | 'vw';
type CSSValue<U extends CSSUnit> = `${number}${U}`;

type PixelValue = CSSValue<'px'>;
// Type: `${number}px` ✅

type RemValue = CSSValue<'rem'>;
// Type: `${number}rem` ✅

// Validación en compilación
const width: PixelValue = '100px'; // ✅
const height: RemValue = '2rem'; // ✅
// const invalid: PixelValue = '100rem'; // ❌ Error: no es px
```

---

## Environment Variables

Config type-safe con variables de entorno prefijadas.

```typescript
type EnvPrefix = 'APP' | 'DB' | 'REDIS' | 'AWS';
type EnvVariable<P extends EnvPrefix, Name extends string> = `${P}_${Uppercase<Name>}`;

type AppPort = EnvVariable<'APP', 'port'>;
// Type: 'APP_PORT' ✅

type DbHost = EnvVariable<'DB', 'host'>;
// Type: 'DB_HOST' ✅

type RedisUrl = EnvVariable<'REDIS', 'url'>;
// Type: 'REDIS_URL' ✅

// Config type-safe con env vars
interface EnvConfig {
  [EnvVariable<'APP', 'port'>]: number;
  [EnvVariable<'APP', 'name'>]: string;
  [EnvVariable<'DB', 'host'>]: string;
  [EnvVariable<'DB', 'port'>]: number;
  [EnvVariable<'REDIS', 'url'>]: string;
}

// Usar: nombres garantizados
const config: EnvConfig = {
  APP_PORT: 3000, // ✅
  APP_NAME: 'My App', // ✅
  DB_HOST: 'localhost', // ✅
  DB_PORT: 5432, // ✅
  REDIS_URL: 'redis://localhost:6379', // ✅
  // INVALID_KEY: '', // ❌ Error: no es env var válida
};
```

---

## Pattern Matching & Extraction

Usar `infer` con template literals para extraer partes de strings.

```typescript
// Extraer versión de ruta
type ExtractVersion<T extends string> =
  T extends `/api/v${infer V extends number}/${string}` ? V : never;

type V1 = ExtractVersion<'/api/v1/users'>;
// Type: 1 ✅

type V2 = ExtractVersion<'/api/v2/products'>;
// Type: 2 ✅

type NoVersion = ExtractVersion<'/users'>;
// Type: never ✅

// Extraer entity de ruta CRUD
type ExtractEntity<T extends string> =
  T extends `/${infer E}` | `/${infer E}/:id` ? E : never;

type Entity1 = ExtractEntity<'/users'>;
// Type: 'users' ✅

type Entity2 = ExtractEntity<'/products/:id'>;
// Type: 'products' ✅

// Parsear query params
type ParseQueryParam<T extends string> =
  T extends `${infer Key}=${infer Value}` ? { [K in Key]: Value } : never;

type Parsed = ParseQueryParam<'name=John'>;
// Type: { name: 'John' } ✅
```

---

## Resumen: Lo Que Aprendiste

- ✅ **Sintaxis:** `` `string ${Type} string` `` (template strings para tipos)
- ✅ **Unions:** Generan combinaciones automáticamente (producto cartesiano)
- ✅ **Intrinsic Types:** Uppercase, Lowercase, Capitalize, Uncapitalize
- ✅ **Event Names:** Patrón `entity.action` con validación
- ✅ **Routes:** API versioning, CRUD patterns, type-safe
- ✅ **Query Builders:** Operadores tipo-seguros por tipo de field
- ✅ **Headers & CSS:** Validación de standards web
- ✅ **Environment Variables:** Config con prefijos garantizados
- ✅ **Pattern Extraction:** `infer` + template literals para parsing
- ✅ **Real-World:** Event emitters, routers, validators — todo generado

**Lo importante:** Template literal types = **strings como tipos de primera clase**. No son "cualquier string", son strings específicos, validados, escalables. ¡La máxima especificidad sin sacrificar flexibilidad! 🎯

---

## 📋 Checklist: ¿Entendiste Template Literal Types? Tengamos presente lo siguiente:

- ✅ Sintaxis: `` `string ${Type} string` ``
- ✅ Unions generan producto cartesiano
- ✅ Intrinsic string types: Uppercase, Lowercase, Capitalize
- ✅ Event names type-safe: `entity.action` pattern
- ✅ Route definitions: versioning, CRUD, params
- ✅ Query builders: field operators type-safe
- ✅ HTTP headers: standard + custom con prefijos
- ✅ CSS values: units y validación
- ✅ Environment variables: prefijos y nombres garantizados
- ✅ Pattern extraction: `infer` con template literals

**🎯 Próximo paso:** [6️⃣ Decorators Deep Dive](./6.Decorators[TypeScript].md) — Metadata y runtime reflection en NestJS.

**🎯 Recuerda:** Template literal types convierten strings en ciudadanos de primera clase del sistema de tipos. Úsalos para máxima seguridad y escalabilidad.
